...
public class ThongKeKhachHang extends JPanel implements ActionListener {
	...
	private JButton btnXacNhan, btnSoGioNhieuNhat;
...
//		panel_1.add(chonThang);
		...
		btnSoGioNhieuNhat = new JButton("So Gio Nhieu Nhat");
		btnSoGioNhieuNhat.setFont(new Font("Tahoma", Font.PLAIN, 13));
		btnSoGioNhieuNhat.setBackground(Color.WHITE);
		btnSoGioNhieuNhat.setBounds(23, 81, 214, 44);
		panel_1_1.add(btnSoGioNhieuNhat);
		...
		JLabel lblBieuDo = new JLabel("BIEU DO SO GIO DEN NHIEU NHAT CUA KHACH HANG ");
...
//		panel_2.add(pnlChart);
		...
		table.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "STT", "Mã Khach Hang", "Ten Khach Hang", "SDT Khách Hàng", "So Gio Den" }));
		...
		btnSoGioNhieuNhat.addActionListener(this);
	...
	public void loadData(ArrayList<HoaDon> dsHoaDon) {
		xoaALLDuLieuTable();
		int i = 1;
		Map<KhachHang, HoaDon> hoaDonTheoKhachHang = new HashMap<>();
		for (HoaDon hoaDon : dsHoaDon) {
			...
			Object[] data = { i, kh.getMaKH(), kh.getTenKH(), kh.getSdt(), soGioDen };
			model.addRow(data);
			i++;
		}
	...
	public void thongkeHoaDonNgayHienTai(LocalDate now) {
		...
		for (HoaDon hoaDon : dsTrongNgay) {
			...
			ArrayList<ChiTietHoaDon> dsChiTietHoaDon = CTHDDAO.layDSTheoMaHD(hoaDon.getMaHoaDon());
			for (ChiTietHoaDon chiTietHoaDon : dsChiTietHoaDon) {
				double soGio = chiTietHoaDon.getThoiLuong();
				soGioDen += soGio;
			}
			dataset.addValue(soGioDen, "Khach Hang", hoaDon.getMaKhachHang());
		}
		JFreeChart chart = ChartFactory.createBarChart("BIEU DO SO GIO DEN NHIEU NHAT " + String.valueOf(now),
				"KHACH HANG", "SO GIO", dataset, PlotOrientation.VERTICAL, false, false, false);
		panel_2.removeAll();
	...
	public JFreeChart taoBieuDoTheoThang(CategoryDataset dataset, String thang) {
		JFreeChart chart = ChartFactory.createBarChart("Biểu Đồ So Gio Den Quan Thang" + thang, "Khach Hang", "So Gio",
				dataset, PlotOrientation.VERTICAL, false, false, false);
		return chart;
	}
	...
	private CategoryDataset createDataSetTheoThang(ArrayList<HoaDon> dsHoaDonTheoThang) {
		...
		for (int i = 1; i < 32; i++) {
			for (HoaDon hoaDon : dsHoaDonTheoThang) {
				...
				dataset.addValue(soGioDen, hoaDon.getMaKhachHang(), " " + i);
			}
		}
		return dataset;
	}
	...
	public JFreeChart taoBieuDoTheoNam(CategoryDataset dataset, String nam) {
		JFreeChart chart = ChartFactory.createBarChart(" Biểu Đồ Thong Ke Khach Hang Co Gio Den Quan Nhieu Nhat" + nam,
				"Khach Hang", "So Gio", dataset, PlotOrientation.VERTICAL, false, false, false);
		return chart;
	}
	...
	public JFreeChart taoBieuDoTheoNgay(CategoryDataset dataset, String Ngay) {
		JFreeChart chart = ChartFactory.createBarChart("Biểu Đồ Thong Ke Khach Hang Co Gio Den Quan Nhieu Nhat " + Ngay,
				"Khach Hang", "So Gio", dataset, PlotOrientation.VERTICAL, false, false, false);
		return chart;
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if (o.equals(btnSoGioNhieuNhat)) {
			if (cmbLuaChon.getSelectedItem().equals("Theo tháng")) {
